...
def upd_db():
    ...
    pk = 'C:/Users/днс/Desktop/vk_p/pv/gho.db'
...
def wall_get():
    ...
    if 'text' in wa['response']['items'][0]:
        ...
        block_list=['тогда тебе','записывайтесь','ждём вас','ждем вас',
            'перейдите по ссылке','наш адрес', 'требуются',
            'записывайся','записывайся по номеру','переходите по ссылке',
            'предложение ограниченно','цены вырастут','заходите в сообщество',
            'многое другое в паблике','самые низкие цены','низкие цены',
            'переходи по ссылки в комментариях','все подробности в паблике',
            'подробная информация в группе','качественно','недорого',
            'качественно и не дорого','пишите и звоните',' подписывайся на канал',
            'подписывайся','спешите обновить','у нас вы найдете','сделайте заказ',
            'сделайте заказ прямо сейчас','скидка']
    ...
    if copy_history == False and marked_as_ads == False and block_comments == False and post_ads == False:
        if 'attachments' in wa['response']['items'][0]:
            for i in wa['response']['items'][0]['attachments']:
                ...
                if _type == 'photo' or _type =='video':
                    ...
                    if _type == 'photo' and 'album_id' in i['photo']:
                        ...
                        print(f"{_type}{i[_type]['owner_id']}_{i[_type]['id']}_{i[_type]['access_key']}")


                    if _type == 'photo' and not 'album_id' in i['photo']:
                        sp_tg.append(f"https://vk.com/{_type}{i[_type]['owner_id']}_{i[_type]['id']}")
                        sp_vk.append(f"{_type}{i[_type]['owner_id']}_{i[_type]['id']}_{i[_type]['access_key']}")
        ...
        if len(sp_vk)>0 and text_in_post == False and signer_id == False:
            if geo == False:
                ...
                if audio == False and poll == False and album == False and doc == False:
                    send_med_gr(sp_tg,cop,text_tg,id['post_id'])
                else:
                    text_tg+= f"<a href='{cop}'>Оригинальный пост</a>\n<a href='https://vk.com/wall{id_gossip_nf_clone}_{id['post_id']}'>Скопированный пост</a>\n\n<a href='https://vk.com/gossipnfclone'>Gossip NF CLONE</a>\n<a href='https://vk.com/gossipnf'>Gossip NF</a>"
                    bot.send_message(id_tg_group,text_tg, parse_mode='HTML',disable_web_page_preview = True)
        ...
        if len(sp_vk)<1 and text_in_post == True and signer_id == False:
            if geo == False:
                ...
                text_tg+= f"<a href='{cop}'>Оригинальный пост</a>\n<a href='https://vk.com/wall-214698143_{id['post_id']}'>Скопированный пост</a>\n\n<a href='https://vk.com/gossipnfclone'>Gossip NF CLONE</a>\n<a href='https://vk.com/gossipnf'>Gossip NF</a>"
                bot.send_message(id_tg_group,text_tg, parse_mode='HTML',disable_web_page_preview = True)
                print('пост 5')
...
def dp_c():
    ...
    id=6771
    cursor.execute(f"UPDATE `name` SET `new_id`={id}")
    connect.commit()
